  ...
  state = {
    ...
    loadData () {
      if (this.props.match.params.slug ) {
                axios.get( '/api/store/' + this.props.match.params.slug)
                    .then(response => {
                        this.setState({ loadedPost: response.data });
                        console.log("single store" + response.data); 
                    } );
        }
     }
    ...
    <div className="single">
      <div className="single__hero"><img className="single__image" src={img1} alt="img1"/>
          <h2 className="title title--single name">{this.state.loadedPost.name}</h2>
      </div>
    </div>
