  ...
  state = {
    ...
    loadData () {
      if (this.props.match.params.slug ) {
                axios.get('/api/delivery/' + this.props.match.params.slug)
                    .then(response => {
    ...
    <div className="single">
      <div className="single__hero"><a href={src}><img className="single__map" src={img2} alt="map"/></a>
          <h2 className="title title--single name"><a href="">{this.state.loadedPost.name}</a></h2>
      </div>
    </div>
    <div className="single__details inner"><img className="single__map" src={img2} alt="map"/>
